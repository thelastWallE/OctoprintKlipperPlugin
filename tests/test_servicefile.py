...
class TestSaveServicefile:
    def test_save_writes_current_and_deploys(self, plugin_self, tmp_path):
        ...
        with mock.patch(
            ...
            result = servicefile.save_servicefile(
                plugin_self, new_content, str(dest)
            )
        assert result["status"] == "success"
        # current holds the CURRENT content
        assert (
            data_dir / "current" / "servicefile" / "klipper.service"
    ...
    def test_save_returns_password_required_when_not_cached(self, plugin_self, tmp_path):
        from unittest import mock

...
class KlipperLogAnalyzer:
    ...
    def parse_log(self, logname, mcu):
        ...
        try:
            ...
            extra.log_error(self, False, "Couldn't open log file: {}".format(logname))
            print("Couldn't open log file")
        return out
    ...
    def find_print_restarts(self, data):
        ...
        return {
            sampletime: 1
            for stall, samples in list(runoff_samples.values())
    ...
    def plot_mcu(self, data, maxbw):
        ...
        for d in data:
            ...
            if hb >= self.MAXBUFFER or st in sample_resets:
                ...
                hb = 100.0 * (self.MAXBUFFER - hb) / self.MAXBUFFER
            hostbuffers.append(hb)
            times.append(st)
            bwdeltas.append(100.0 * (bw - lastbw) / (maxbw * timedelta))
            loads.append(100.0 * load / self.TASK_MAX)
            awake.append(100.0 * float(d.get("mcu_awake", 0.0)) / self.STATS_INTERVAL)
            lasttime = st
            lastbw = bw
